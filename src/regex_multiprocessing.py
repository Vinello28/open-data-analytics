...
def process_chunk_ai(chunk):
    ...
    """
    if "DESCRIZIONE_PROGETTO" not in chunk.columns or "CLASSIFICAZIONE" not in chunk.columns:
        return pd.DataFrame()
...
def process_file_ai(filepath):
    ...
    try:
        chunk_iter = pd.read_csv(filepath, chunksize=50000, 
                                 usecols=lambda c: c in ["DESCRIZIONE_PROGETTO", "CLASSIFICAZIONE"], 
                                 low_memory=False, 
                                 on_bad_lines='skip')
        for chunk in chunk_iter:
...
def run_ai_extraction(data_dir, target_per_class=5000):
    files = glob.glob(os.path.join(data_dir, "*.csv"))
    print(f"Trovati {len(files)} file CSV da processare (solo AI).")
    ...
    with Pool(num_workers) as pool:
        for file_res in pool.imap_unordered(process_file_ai, files):
            if not file_res.empty:
                final_df = pd.concat([final_df, file_res], ignore_index=True)
                final_df = final_df.drop_duplicates(subset=["DESCRIZIONE_PROGETTO"])
                ...
                print(f"Campioni AI aggregati - Formazione: {count_formazione}, Implementazione: {count_implementazione}")
                if count_formazione >= target_per_class and count_implementazione >= target_per_class:
                    print("Raggiunto il target desiderato per entrambe le classi AI!")
                    pool.terminate()
                    break
